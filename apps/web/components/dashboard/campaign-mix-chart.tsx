...
const NEEDS_REVIEW_COLOR = "#94a3b8"; // muted — 검토 필요는 항상 회색으로 구분
...
export function CampaignMixChart({
  ...
  /** campaign_tag_id(string) 또는 "NEEDS_REVIEW"를 키로 하는 카운트. */
  mix: Record<string, number>;
  campaignTags: CampaignTag[];
  title?: string;
}) {
  const nameOf = (key: string) =>
    key === "NEEDS_REVIEW" ? "검토 필요" : (campaignTags.find((t) => t.id === key)?.name ?? "알 수 없음");
  const colorOf = (key: string, index: number) =>
    key === "NEEDS_REVIEW" ? NEEDS_REVIEW_COLOR : PALETTE[index % PALETTE.length];
  ...
  if (total === 0) {
    return (
      <div className="rounded-2xl border border-border bg-white p-5">
        ...
        <p className="mt-1 text-[11px] text-muted">이번 주 신규/재집행 소재 기준 · 현재 태그 분류 기준</p>
        <p className="mt-3 text-xs text-muted">이 기간에 신규/재집행된 소재가 없습니다.</p>
      </div>
    );
  }
