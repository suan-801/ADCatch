...
사용법:
  ...
  cd apps/api && python -m scripts.process_pending_video_keyframes --competitor-id <uuid> --limit 20

GitHub Actions workflow_dispatch(.github/workflows/process-video-keyframes.yml)가 이 스크립트를
그대로 재사용한다 — limit/competitor_id를 workflow input으로 받아 이 스크립트의 인자로 전달한다.
"""
...
def run(limit: int | None, competitor_id: uuid.UUID | None) -> None:
    db = SessionLocal()
    try:
        summary = process_pending_video_keyframes(db, competitor_id=competitor_id, limit=limit)
        print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--competitor-id", type=str, default=None, help="이 경쟁사(브랜드)의 PENDING 소재만 처리(생략 시 전체)"
    )
    args = parser.parse_args()
    run(args.limit, uuid.UUID(args.competitor_id) if args.competitor_id else None)
