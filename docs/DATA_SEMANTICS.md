...
## 10. VIDEO/IMAGE 미디어 판별 (2026-09, 2026-09-23 단순화)

**2026-09-23 재설계**: `CAROUSEL` 포맷과 ffmpeg keyframe 파이프라인(영상 다운로드+ffprobe+
ffmpeg×4+Storage 업로드×4+재시도 lifecycle)을 완전히 제거했다. ADCatcher의 목적은 영상 분석
서비스가 아니라 "어떤 광고가 살아있고 어떤 패턴/캠페인으로 운영되는지 빠르게 보는 트래커"다 —
직전 라운드(2026-09-22)에 도입한 display_format/DCO/CAROUSEL/MULTI_IMAGES/DPA 3-way 분류는
실패 지점이 많고 이 목적에 비해 과한 복잡도였다. `AdFormat`은 이제 `IMAGE`/`VIDEO` 2가지뿐이다.

`Ad.image_url`은 포맷과 무관하게 계속 "대표 썸네일 1장"으로 쓰인다(기존 갤러리 동작 불변).
`video_url`은 VIDEO 포맷의 **참고용 원본 URL**(HD 우선, SD fallback)이다 — UI에서 재생하지
않으므로(원본을 보고 싶으면 "Meta에서 보기" 버튼으로 이동) signed URL 만료를 신경 쓸 필요가
없다. `media_items`/`keyframe_urls`/`keyframe_status`/`keyframe_retry_count`/`keyframe_error`
컬럼은 DB에 legacy로 남아있지만 API/서비스 어디에서도 더 이상 채우거나 읽지 않는다(운영 안정화
후 별도 migration으로 drop 검토, `db/migrations/003_simplify_media_format.sql` 참고).

**format 판별 규칙(`app.services.ad_library_collector.classify_and_extract_media()`)**: VIDEO는
아주 명확한 경우에만 VIDEO로 판정하고, 그 외 전부(카드가 여러 장인 캐러셀, 카드 안에 영상이
섞여 있는 경우 포함) IMAGE로 취급한다.
1. `_raw_display_format()`(item 또는 snapshot의 `display_format`/`displayFormat`, 대소문자
   ...
   케이스가 재현 사례(`ad_archive_id=1411948807546506`, 영상 광고를 cards 배열로 감싸 내려주는
   경우)다.
2. `snapshot.videos`가 실제로 존재하고 유효한 video URL이 있으면 → `VIDEO`(display_format
   신호가 없거나 다른 값이어도).
3. `snapshot.cards`가 **정확히 1개**이고 그 카드에 video URL이 있으면 → `VIDEO`(Apify가 단일
   영상 광고를 카드 1개짜리 배열로 감싸는 케이스 — 카드가 1개뿐이면 "여러 장 넘겨보는 캐러셀"
   이라는 의미 자체가 없다).
4. 그 외 전부 → `IMAGE`. 캐러셀(카드 2개 이상)은 첫 카드의 대표 이미지 1장만 쓴다 — 카드 안에
   영상이 섞여 있어도(카드 하나가 영상이라도) `VIDEO`로 승격하지 않는다. `display_format`이
   명시적으로 VIDEO를 주장해도 실제 video URL을 하나도 찾지 못하면(메타데이터 불일치) 이
   규칙으로 내려간다.

VIDEO의 대표 썸네일 우선순위: `video_preview_image_url → original_image_url →
resized_image_url`(videos[] 노드 우선, 그다음 cards[] 노드) → cards의 첫 이미지 →
`snapshot.images` 첫 이미지 → `NULL`. IMAGE(캐러셀 포함)의 대표 이미지는 첫 카드 이미지, 없으면
`snapshot.images` 첫 이미지다.

**여전히 검증되지 않은 부분**: 이 저장소에는 APIFY_TOKEN/.env가 없어(2026-09-23 재확인)
1411948807546506의 실제 raw item을 직접 조회하지 못했다. 위 규칙은 사용자가 이번 요청에서
직접 지정한 분류 기준이며, 실제 Apify 응답 필드명/값과 정확히 일치하는지는 아래 진단 로깅으로
계속 재검증이 필요하다.
...
`images_count`/`classified_format`/`classification_reason`을 structured log로 남긴다(URL 원문은
남기지 않음). `classification_reason` 값 목록: `display_format_video`(규칙 1)/
`videos_present`(규칙 2)/`single_card_video`(규칙 3)/`image_default`(규칙 4).

**기존(이전 3-way 규칙으로 잘못 판정됐던) 소재의 backfill/교정**: 별도 백필 스크립트를 두지
않는다. 다음 수집에서 그 소재가 다시 발견될 때(`ad_sync.synchronize_ad_status`의 기존 소재
재발견 분기):
- **fresh raw evidence(`item.video_url` 또는 `item.image_url`이 비어있지 않음)가 명확하고
  `item.format != existing.format`이면 `existing.format`을 최신 판정으로 교정한다.** raw
  evidence가 비어있거나 불확실하면(빈 snapshot 등) 절대 덮어쓰지 않는다.
- `video_url`은 format이 (교정 후 기준으로) `VIDEO`인 동안 fresh 값이 있으면 항상 최신으로
  갱신한다(재생하지 않으므로 만료 걱정이 없다). format이 더 이상 VIDEO가 아니게 교정되면 더
  이상 대표 영상이 아니므로 비운다.
...
**마이그레이션(`db/migrations/003_simplify_media_format.sql`)**: 기존 DB의 `CAROUSEL` row는
전부 `IMAGE`로 일괄 다운그레이드한다(보수적 접근 — `media_items`를 뒤져 "진짜 VIDEO였을 것"을
추측하지 않는다). 진짜 영상 광고였던 row는 다음 Apify 재수집에서 위 규칙에 따라 자연스럽게
`VIDEO`로 재교정된다 — 이 마이그레이션은 raw item을 직접 보지 못하므로 추측성 승격을 하지
않는다.
